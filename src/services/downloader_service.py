...
class DownloaderService:
    ...
    def generate_search_candidates(cls, track_info: Dict) -> List[str]:
        ...
        # Priority 1: Official Audio / Studio release
        if artist and title:
    ...
    @classmethod
    def score_studio_entry(cls, entry: Dict, artist: str = '', title: str = '') -> int:
        """
        Ranks candidate YouTube videos ensuring strict title/artist relevance,
        prioritizing pure Studio Master / Topic audio and eliminating wrong song matches.
        """
        if not entry:
            return -999
        ...
        # 1. Studio Topic Channel (+80) - Official label releases (only if title matched)
        if ('topic' in uploader or 'provided to youtube by' in desc or 'auto-generated by youtube' in desc) and score > 0:
            score += 80

        # 2. Official Studio Audio / Visualizer / Lyric Track (+60)
        if 'official audio' in t:
            score += 60
        elif 'audio' in t and ('official' in t or 'lyrics' in t):
            score += 45
        if 'official visualizer' in t or 'visualizer' in t:
            score += 35
        if 'lyric video' in t or 'lyrics' in t:
            score += 30

        # 3. Penalize Live, Karaoke, Covers, Reaction, Parody (-300)
        if any(bad in t for bad in [
            ...
            'parody', 'fan made', 'slowed', 'reverb', 'sped up', '1 hour', '10 hours'
        ]):
            score -= 300

        # 4. Official Music Video is fine as long as title matched
        if any(mv_word in t for mv_word in ['official music video', 'official video', 'mv', '[mv]', '(mv)']):
            score += 20

        # 5. Reasonable duration preference (1.5 - 7 mins)
        if 90 <= duration <= 480:
            score += 20
                                ...
                                if search_res:
                                    ...
                                    if entries:
                                        scored_entries = sorted(
                                            entries,
                                            key=lambda e: cls.score_studio_entry(e, artist=artist, title=title),
                                            reverse=True
                                        )
